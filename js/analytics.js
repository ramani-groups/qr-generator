(() => {
  ...
  function jsonpFallback(p){return new Promise((resolve,reject)=>{const cb=`ramaniAnalytics_${Date.now()}_${Math.random().toString(36).slice(2)}`,s=document.createElement('script');let done=false,timer;const finish=(e,d)=>{if(done)return;done=true;delete window[cb];s.remove();clearTimeout(timer);e?reject(e):resolve(d)};window[cb]=d=>finish(null,d);const u=new URL(cfg.appsScriptUrl);Object.entries({...p,callback:cb}).forEach(([k,v])=>u.searchParams.set(k,v));s.src=u.toString();s.onerror=()=>finish(new Error('JSONP was blocked by the browser.'));document.head.appendChild(s);timer=setTimeout(()=>finish(new Error('JSONP request timed out.')),12000);});}

  function jsonp(p){
    if(!cfg.appsScriptUrl)return Promise.reject(new Error('Tracking service is not configured.'));
    return new Promise((resolve,reject)=>{const requestId=`ramaniAnalyticsFrame_${Date.now()}_${Math.random().toString(36).slice(2)}`,f=document.createElement('iframe');let done=false;f.hidden=true;f.style.display='none';const finish=(e,d)=>{if(done)return;done=true;clearTimeout(timer);window.removeEventListener('message',onMessage);f.remove();e?reject(e):resolve(d)};const onMessage=e=>{const d=e.data;if(!d||d.source!=='ramani-qr-api'||d.requestId!==requestId)return;finish(null,d.payload)};window.addEventListener('message',onMessage);const u=new URL(cfg.appsScriptUrl);Object.entries({...p,transport:'iframe',requestId}).forEach(([k,v])=>u.searchParams.set(k,v));f.src=u.toString();document.body.appendChild(f);const timer=setTimeout(()=>finish(new Error('Iframe transport timed out.')),12000);}).catch(first=>jsonpFallback(p).catch(second=>{throw new Error(`Could not reach tracking service. Iframe: ${first.message} JSONP: ${second.message}`);}));
  }
