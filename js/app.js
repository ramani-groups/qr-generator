(() => {
  ...
  function jsonpFallback(params) {
    return new Promise((resolve, reject) => {
      const callback = `ramaniQrCb_${Date.now()}_${Math.random().toString(36).slice(2)}`;
      const script = document.createElement('script');
      const timeout = setTimeout(() => cleanup(new Error('Tracking service timed out.')), 10000);
      ...
      script.onerror = () => cleanup(new Error('JSONP was blocked by the browser.'));
      document.head.appendChild(script);
    });
  }

  function jsonp(params) {
    if (!apiConfigured()) return Promise.reject(new Error('Tracking service is not configured.'));
    return new Promise((resolve, reject) => {
      const requestId = `ramaniFrame_${Date.now()}_${Math.random().toString(36).slice(2)}`;
      const frame = document.createElement('iframe');
      frame.hidden = true;
      frame.setAttribute('aria-hidden', 'true');
      frame.style.display = 'none';
      let settled = false;
      const timer = setTimeout(() => finish(new Error('Iframe transport timed out.')), 12000);
      function cleanup() {
        clearTimeout(timer);
      ...
      function onMessage(event) {
        ...
        if (!data || data.source !== 'ramani-qr-api' || data.requestId !== requestId) return;
        finish(null, data.payload);
      }
      window.addEventListener('message', onMessage);
      const url = new URL(cfg.appsScriptUrl);
      Object.entries({ ...params, transport: 'iframe', requestId }).forEach(([k, v]) => url.searchParams.set(k, v));
      frame.src = url.toString();
      document.body.appendChild(frame);
    }).catch(firstError => jsonpFallback(params).catch(secondError => {
      throw new Error(`Could not reach the tracking service. Iframe: ${firstError.message} JSONP: ${secondError.message}`);
    }));
  }
  ...
  function buildTrackedPayload(actionPayload) {
    ...
    return { payload: `${base}/go.html?n=${encodeURIComponent(name)}&t=${encodeURIComponent(state.type)}&k=${encodeURIComponent(state.token)}&d=${encodeURIComponent(data)}`, error: '' };
  }
  ...
  function setStatus(error) {
    ...
    [els.downloadPng, els.downloadSvg, els.copyContent].forEach(b => { b.disabled = !state.valid; });
  ...
  async function checkName() {
    ...
    nameTimer=setTimeout(async()=>{
      try {
        const result=await jsonp({action:'checkName',name});
  ...
  async function ensureRegistered() {
    ...
    const result=await jsonp({action:'register',name,type:state.type,token:state.token});
    if (!result?.ok) {
  ...
  function resetAll() {
    ...
    state.registeredName=''; state.registeredFingerprint=''; state.token=makeToken(); state.nameStatus='unknown'; els.nameStatus.textContent='Required for tracking and reports.'; els.nameStatus.className='name-status'; setType(DEFAULTS.type); showToast('Settings reset');
  }
  ...
  els.qrName.addEventListener('input',()=>{ state.token=makeToken(); checkName(); });
