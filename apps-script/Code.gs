...
function doGet(e) {
  try {
    ...
    return output_(result, p.callback, p.transport, p.requestId);
  } catch (err) {
    return output_({
      ok: false,
      message: err && err.message ? err.message : String(err)
    }, e && e.parameter ? e.parameter.callback : '', e && e.parameter ? e.parameter.transport : '', e && e.parameter ? e.parameter.requestId : '');
  }
}
...
function ensureSheet_(ss, name, headers) {
  ...
  // Always repair the header row instead of failing on old/partial headers.
  // This keeps rows 2+ untouched.
  const headerRange = sheet.getRange(1, 1, 1, headers.length);
  const existing = headerRange.getDisplayValues()[0];
  const normalizedExisting = existing.map(function(v) {
    return String(v || '').trim().replace(/\s+/g, ' ');
  });
  const normalizedRequired = headers.map(function(v) {
    return String(v).trim().replace(/\s+/g, ' ');
  });

  let mismatch = false;
  for (let i = 0; i < headers.length; i++) {
    if (normalizedExisting[i] !== normalizedRequired[i]) {
      mismatch = true;
      break;
    }
  }
...
function output_(payload, callback, transport, requestId) {
  const json = JSON.stringify(payload);
  const mode = String(transport || '').trim().toLowerCase();

  // Preferred transport for static sites: load the Apps Script web app in a
  // hidden iframe and return the result to the parent page with postMessage.
  // This avoids browsers/extensions that block cross-origin JSONP script tags.
  if (mode === 'iframe') {
    const id = String(requestId || '').trim();
    const safeJson = json.replace(/</g, '\\u003c').replace(/>/g, '\\u003e').replace(/&/g, '\\u0026');
    const html = '<!doctype html><html><head><meta charset="utf-8"></head><body>' +
      '<script>(function(){var msg={source:"ramani-qr-api",requestId:' + JSON.stringify(id) + ',payload:' + safeJson + '};' +
      'try{window.top.postMessage(msg,"*");}catch(e){try{window.parent.postMessage(msg,"*");}catch(_){}}})();<\/script></body></html>';
    return HtmlService
  ...
  // JSONP fallback for browsers that allow third-party script loading.
